...
def load_and_prepare_data(path: str, test_size: float = 0.25, random_state: int = 42):
    logger.info(f"Loading data from {path}")
    df = pd.read_csv(path, parse_dates=['Date'])
    logger.info(f"Raw data shape: {df.shape}")

    dep_var = "r_posn"
    X = df.drop([dep_var, 'Date'], axis=1, errors='ignore')
    y = df[dep_var]
...
def main(args):
    X_train, X_test, y_train, y_test = load_and_prepare_data(args.data_path, args.test_size, args.random_state)
